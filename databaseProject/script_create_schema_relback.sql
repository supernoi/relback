...
Alter de diskgroup or filesystem of Datafile 

*/

CREATE TABLESPACE "TBS_RELBACK" SIZE 256M AUTOEXTEND ON NEXT 128M MAXSIZE 10240M 
...
ALTER TABLE RELBACK.USERS ADD CONSTRAINT CK_USERS_THEME_PREFERENCE CHECK ( THEME_PREFERENCE IN ('light', 'dark', 'auto')) ;
ALTER TABLE RELBACK.USERS ADD CONSTRAINT CK_USERS_LANGUAGE_PREFERENCE CHECK ( LANGUAGE_PREFERENCE IN ('en', 'pt')) ;
ALTER TABLE RELBACK.USERS ADD CONSTRAINT CK_USERS_NOTIFICATIONS_ENABLED CHECK ( NOTIFICATIONS_ENABLED IN (0, 1)) ;
...
/*
Create views to access data of RMAN catalog
*/
...
CREATE OR REPLACE FORCE VIEW "RELBACK"."VW_RMAN_BACKUP_JOB_DETAILS" ("DB_NAME",
...
CREATE OR REPLACE FORCE VIEW "RELBACK"."VW_RMAN_BACKUP_SUBJOB_DETAILS" (
...
CREATE OR REPLACE FORCE VIEW "RELBACK"."VW_RMAN_OUTPUT" ("DB_KEY",
